...
const URL1 = `http://localhost:8080/api/v1/admin/Chefs`;//pour faire lintersection
...
const TabDeSuivi = () => {
  ...
  const [chefs, setChefs] = useState(null); // Étape 1
  ...
  {Array.isArray(chefs) &&
    chefs.map((chef) => {
      const matchedVente = chef.table_de_vente.find(
        (venteItem) => venteItem.id === vente.id
      );
      // Assuming chef.nom contains the name of the chef, you can display it like this:
         return matchedVente ? <div key={chef.id}>{chef.nom}</div> : null;
    })}
</td>
