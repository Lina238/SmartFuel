...
const URL = `http://localhost:8080/api/v1/${lowercaseRole}/Table_de_vente`;
...
const TabDeSuivi = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [gisementResponse, chefsResponse] = await Promise.all([
          fetch(URL, getDefaultFetchConfig('GET')).then((res) => res.json()),
          fetch(URL1, getDefaultFetchConfig('GET')).then((res) => res.json()),
        ]);

        const combinedData = gisementResponse.map((item) => {
          const newTableDeVente = item.table_de_vente.map((vente) => {
            const correspondingChef = chefsResponse.find(
              (chef) => chef.id === vente.id
            );
            const user_opperation = correspondingChef ? correspondingChef.nom : '';
            return { ...vente, user_opperation };
          });
          return { ...item, table_de_vente: newTableDeVente };
        });

        setSuivData(combinedData);
