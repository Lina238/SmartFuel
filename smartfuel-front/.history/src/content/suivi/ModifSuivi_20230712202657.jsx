...
const ModifSuivi = () => {
    ...
    useEffect(()=> {
        fetch(URL+'/'+suivid,getDefaultFetchConfig('GET')).then((res)=> {
            return res.json();
        }).then((resp)=> {
            setNom(item.id_gisement_distributeur.id_distributeur.nom);
            setCompteur_actuel(resp.compteur_actuel) ;
            setCompteur_final(resp.compteur_final);
            setQuantite(resp.quantite);
            setEspace_libre(resp.id_gisement_distributeur.idgisement.espace_libre);
            setUser_opperation(resp.user_opperation.nom);
            setDate_de_creation(resp.date_de_creation);
            setDate_de_modification(resp.date_de_modification);
            setMontant_actuel(resp.montant);
