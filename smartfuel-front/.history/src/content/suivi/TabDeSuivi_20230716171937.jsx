...
const URL1 = 'http://localhost:8080/api/v1/cheftable'; // Replace this with the correct endpoint for the chef table
...
const TabDeSuivi = () => {
    ...
    fetch(URL, getDefaultFetchConfig('GET'))
      ...
      .then((resp) => {
        ...
        const updatedSuivData = resp.map((item) => {
          const userOpperation = getUserOpperation(item.table_de_vente);
          return { ...item, user_opperation: userOpperation };
        });
  ...
  const getUserOpperation = (ventes) => {
    const userId = ventes[0]?.id;
    if (userId && userData[userId]) {
      return userData[userId];
    }
    return 'Unknown'; // If user not found or ventes array is empty
  };
