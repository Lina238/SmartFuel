...
const TabDeSuivi = () => {
  ...
  useEffect(() => {
    fetch(URL, getDefaultFetchConfig('GET'))
      .then((res) => res.json())
      .then((resp) => {
        // Update SuivData with user_opperation field
        const updatedSuivData = resp.map((item) => {
          const userOperation = getUserOperation(item.table_de_vente);
          return { ...item, user_opperation: userOperation };
        });
        setSuivData(updatedSuivData);
      })
      .catch((err) => {
  ...
  // Function to find the user_opperation based on vente id
  const getUserOperation = (venteId) => {
    // Find the correct item in suivData that contains the vente with matching ID
    const itemWithVente = suivData.find((item) =>
      item.table_de_vente.some((vente) => vente.id === venteId)
    );

    if (itemWithVente) {
      const userId = itemWithVente.id;
      if (userId && distributorNames[userId]) {
        return distributorNames[userId];
      }
    }
    return 'Unknown'; // If user not found or vente with matching ID not found
  };
