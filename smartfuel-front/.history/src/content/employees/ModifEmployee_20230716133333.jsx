...
const ModifEmployee = () => {
  ...
  const [chef, setChef] = useState({ id: "", username: "" });
  ...
  useEffect(() => {
    fetch(URL + "/" + empid, getDefaultFetchConfig("GET"))
      ...
      .then((resp) => {
       ...
       if(resp.chef.num!=null){ setChef(resp.chef.username)};
      })
      .catch((err) => {
        console.log(err.message);
      });
                  ...
                  <div style={{ margin: "10px" }} className="col-lg-12">
                    <div className="form-group">
                      ...
                      <select
                        value={chef.username}
                        onChange={(e) => {
                          const selectedChef = chefsOptions.find((item) => item.username === e.target.value);
