...
const AjoutProduit = () => {
  ...
  const handlesubmit = (e) => {
    ...
    if (!selectedDis ||!selectedgis ) {
      ...
      const selectedDisObject = selectedDis.find((item) => item.nom === type);
      const selectedgisObject = selectedgis.find((item) => item.type === type);
      if (!selectedDisObject || !selectedgisObject) {
        alert('Veuillez sélectionner des elements');
        return;
      }
    
      const gisData = {
        idgisement:{id:selectedgisObject},
        iddistributeur: {id:selectedDisObject}, 
