  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      svg
        ...
        .append('rect')
        .attr('class', 'bar')
        .attr('x', (d, i) => x(formatDateLabel(d.periode)) + margin.left) // Use formatDateLabel to convert date to Date object
        .attr('y', (d) => y(Number(d.sommeMontant)) + margin.top) // Use Number() to convert sommeMontant to a number
        .attr('width', x.bandwidth())
        .attr('height', (d) => height - y(Number(d.sommeMontant))) // Use Number() to convert sommeMontant to a number
        .attr('fill', '#FFA500');
    }
  }, [data]);
