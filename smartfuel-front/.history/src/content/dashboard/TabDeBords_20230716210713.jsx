...
const TabDeBords = () => {
  ...
  const sommeMontantList = data.map((item) => Number(item.somme_montant));
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      svg
        ...
        .attr('y', (d) => y(Number(d.somme_montant)) + margin.top) // Convert to number using Number()
        .attr('width', x.bandwidth())
        .attr('height', (d) => height - y(Number(d.somme_montant))) // Convert to number using Number()
        .attr('fill', '#FFA500');
    }
  }, [data]);
