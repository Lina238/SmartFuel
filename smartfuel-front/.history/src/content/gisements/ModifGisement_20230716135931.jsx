...
const ModifEmployee = () => {
  ...
  const [chef, setChef] = useState(null); // Utilise null comme valeur par défaut
  ...
  useEffect(() => {
    fetch(URL + "/" + empid, getDefaultFetchConfig("GET"))
      ...
      .then((resp) => {
        ...
        setChef(resp.chef || null); // Utilise null si chef est null
      })
      .catch((err) => {
        console.log(err.message);
      });
  ...
  const handlesubmit = (e) => {
    ...
    const empData = { username, password, role, chef: chef.id }; // Utilise chef.id pour l'ID du chef
                ...
                <div className="row">
                  ...
                  <div style={{ margin: "10px" }} className="col-lg-12">
                    <div className="form-group">
                      ...
                      <select
                        value={chef ? chef.id : ""} // Utilise chef.id pour la valeur de la liste déroulante
                        onChange={(e) => {
                          const selectedChefId = e.target.value;
                          const selectedChef = chefsOptions.find((item) => item.id === parseInt(selectedChefId));
                          setChef(selectedChef || null); // Utilise chef.id pour mettre à jour le chef
                        }}
                        className="form-control"
                      >
                        <option value="">Sélectionner un chef</option>
                        {chefsOptions.map((item) => (
                          <option key={item.id} value={item.id}> {/* Utilise item.id */}
                            {item.nom}
                          </option>
                        ))}
                      </select>
