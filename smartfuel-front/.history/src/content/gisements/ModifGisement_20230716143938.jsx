...
const ModifEmployee = () => {
  ...
  const [chefs, setChefs] = useState([]); // Liste des chefs
  ...
  useEffect(() => {
    fetch(URL1, getDefaultFetchConfig("GET"))
      .then((res) => res.json())
      .then((data) => {
        setChefs(data); // Met à jour l'état avec les données de l'API
      })
      .catch((err) => {
  ...
  const handleChefChange = (e) => {
    ...
    const selectedChef = chefs.find((item) => item.nom === selectedChefName);
                ...
                <div className="row">
                  ...
                  <div style={{ margin: "10px" }} className="col-lg-12">
                    <div className="form-group">
                      ...
                      <select
                        value={chef ? chef.nom : ""}
                        onChange={handleChefChange} // Utilisez la fonction handleChefChange ici
                        className="form-control"
                      >
