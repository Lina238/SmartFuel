...
const ModifGisement = () => {
  ...
  useEffect(() => {
    fetch(URL + '/' + gisid, getDefaultFetchConfig('GET'))
      ...
      .then((resp) => {
        setNom(resp.nom);
        // setSelectedType(resp.type_gisement);
    ...
    useEffect(() => {
        fetch(URL + '/' + gisid, getDefaultFetchConfig('GET'))
          ...
          .then((resp) => {
            ...
            // Find the selected type in the chefsOptions array and set it as the selectedType
            const selectedTypeObject = chefsOptions.find((item) => item.type === resp.type_gisement.type);
