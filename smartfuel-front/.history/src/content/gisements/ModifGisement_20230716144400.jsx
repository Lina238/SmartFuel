...
const ModifEmployee = () => {
  ...
  const [formData, setFormData] = useState({
    chef: null, // Utilise null comme valeur par défaut
    chefsOptions: [], // Liste des options des chefs
  });
  ...
  useEffect(() => {
    fetch(URL1, getDefaultFetchConfig("GET"))
      .then((res) => res.json())
      .then((data) => {
        setFormData((prevData) => ({ ...prevData, chefsOptions: data })); // Met à jour l'état avec les données de l'API
      })
      .catch((err) => {
  ...
  useEffect(() => {
    fetch(URL + "/" + empid, getDefaultFetchConfig("GET"))
      ...
      .then((resp) => {
        ...
        setFormData((prevData) => ({ ...prevData, chef: resp.chef || null })); // Utilise null si chef est null
      })
      .catch((err) => {
        console.log(err.message);
      });
  ...
  const handleChefChange = (e) => {
    ...
    const selectedChef = formData.chefsOptions.find((item) => item.nom === selectedChefName);
    setFormData((prevData) => ({ ...prevData, chef: selectedChef || null }));
  };
  ...
  const handleSubmit = (e) => {
    ...
    const empData = { username, password, role, chef: formData.chef };
                ...
                <div className="row">
                  ...
                  <div style={{ margin: "10px" }} className="col-lg-12">
                    <div className="form-group">
                      ...
                      <select
                        value={formData.chef ? formData.chef.nom : ""}
                        onChange={handleChefChange}
                        className="form-control"
                      >
                        <option value="">Sélectionner un chef</option>
                        {formData.chefsOptions.map((item) => (
