...
import com.example.SmartFuel.rest.services.Personnels.Personnelrequest;
...
public class SmartFuelController {
	   ...
	   private final Chefserviceimpl chefservice;
        ...
        private final SommeMontantParPeriodeService sommeMontantParPeriodeService;
  	  private final AuthenticationService service;
	@GetMapping("/Personnels")
	  
    @PreAuthorize("hasAuthority('admin:read')")
    public  ResponseEntity<List< Personnel>>  getallpersonnels() {
		List< Personnel> personnels=smartFuelservice.getallpersonnels() ;
	    return new ResponseEntity<>(personnels, HttpStatus.OK);
    }
	  @Autowired  
	  private SommeMontantParPeriodeService s;
    @Autowired
	public SmartFuelController(Chefserviceimpl chefservice, Personnelserviceimpl smartFuelservice,
				...
				SommeMontantParPeriodeService sommeMontantParPeriodeService,AuthenticationService service) {
    	...
    	super();
			...
			this.sommeMontantParPeriodeService = sommeMontantParPeriodeService;
		this.service=service;
    }

	@GetMapping("/Personnels/{id}")
	    @PreAuthorize("hasAuthority('admin:read')")
	    public ResponseEntity<Personnel> getperso(@PathVariable Integer id) {
	    	Personnel perso= smartFuelservice .getperso(id);
    ...
    public ResponseEntity<String> updatepersonnel(@PathVariable Integer id,@RequestBody Personnelrequest perso ) {
    	System.out.println(perso.getUsername());
		  String status=smartFuelservice.updatepersonnel(id,perso);
	  ...
	  public ResponseEntity<String> deletepersonnel(@PathVariable Integer id) {
	    	 String status=smartFuelservice.deletepersonnel(id);
	         return new ResponseEntity<>(status, HttpStatus.OK);
...
////////////////////////////////////////////////gestion des chefs :	
	 ...
	 @GetMapping("/Chefs/{id}")
@PreAuthorize("hasAuthority('admin:read')")
public ResponseEntity<chef> getchef(@PathVariable Integer id) {
	chef perso= chefservice .getchef(id);
    return new ResponseEntity<>(perso, HttpStatus.OK);
}
@PostMapping("/Chefs")
@PreAuthorize("hasAuthority('admin:create')")
@Hidden
public ResponseEntity<String>createchef(  @Valid @RequestBody Chefrequest  perso) {
	
	String ch=chefservice.createchef(perso);
	return new ResponseEntity<>(ch, HttpStatus.CREATED);

}
@PutMapping("/Chefs/{id}")
@PreAuthorize("hasAuthority('admin:update')")
public ResponseEntity<String> updatechef(@PathVariable Integer id,@RequestBody chef perso ) {
	String status=chefservice. updatechef(id,perso);
    return new ResponseEntity<>(status, HttpStatus.OK);
}
...
public ResponseEntity<String>  deletechef(@PathVariable Integer id ) {
	String status=chefservice.deletechef(id);
	  return new ResponseEntity<>(status, HttpStatus.OK);
}
...
////gestion des type_gisement:
 ...
 @GetMapping("/Somee")	
 @PreAuthorize("hasAuthority('admin:read')")
 public ResponseEntity<String> getallsomme() {
	 String dis= sommeMontantParPeriodeService.calculerSommeMontantParPeriode();
	  return new ResponseEntity<>(dis, HttpStatus.OK);
}
